...
export const Checkbox: React.FC<CheckboxProps> = ({ color = "dark", className, ...props }) => (
  <RACheckbox
    className={cn(
      "dark:after:-inset-px group before:-z-10 relative isolate flex size-5 cursor-pointer items-center justify-center rounded-[0.3125rem] border border-zinc-950/15 after:absolute before:absolute after:inset-0 before:inset-0 dark:data-[selected]:after:block dark:after:hidden dark:before:hidden dark:data-[disabled]:data-[selected]:after:hidden sm:size-4 after:rounded-[calc(0.3125rem-1px)] before:rounded-[calc(0.3125rem-1px)] dark:after:rounded-[0.3125rem] dark:border-white/15 dark:data-[disabled]:border-white/20 dark:data-[hovered]:border-white/30 dark:data-[selected]:border-white/5 dark:data-[selected]:data-[hovered]:border-white/5 data-[disabled]:border-zinc-950/25 data-[hovered]:border-zinc-950/30 data-[selected]:border-transparent data-[selected]:data-[hovered]:border-transparent before:bg-white before:data-[selected]:bg-[--checkbox-selected-bg] dark:bg-white/5 dark:data-[disabled]:bg-white/[2.5%] dark:data-[selected]:bg-[--checkbox-selected-bg] data-[disabled]:before:bg-transparent data-[disabled]:bg-zinc-950/5 data-[selected]:bg-[--checkbox-selected-border] data-[disabled]:opacity-50 before:shadow after:shadow-[inset_0_1px_theme(colors.white/15%)] focus:outline-none data-[focused]:outline data-[focused]:outline-2 data-[focused]:outline-sky-500 data-[focused]:outline-offset-2 dark:[--checkbox-selected-bg:theme(colors.zinc.600)] dark:data-[disabled]:[--checkbox-check:theme(colors.white/50%)] dark:forced-colors:[--checkbox-check:HighlightText] dark:forced-colors:[--checkbox-selected-bg:Highlight] dark:forced-colors:data-[disabled]:[--checkbox-check:Highlight] data-[disabled]:[--checkbox-check:theme(colors.zinc.950/50%)] forced-colors:[--checkbox-check:HighlightText] forced-colors:[--checkbox-selected-bg:Highlight] forced-colors:data-[disabled]:[--checkbox-check:Highlight]",
      colorClasses[color],
      className,
    )}
    {...props}
  >
    <svg
      className="size-4 stroke-[--checkbox-check] opacity-0 sm:h-3.5 sm:w-3.5 group-data-[selected]:opacity-100"
      viewBox="0 0 14 14"
      fill="none"
    >
      <path
